...
namespace Aooshi.SnowFlake.Tests
{
    public class GeneratorTests
    {
        ...
        [Fact]
        public void When_converting_from_id_to_long_and_back_then_the_id_is_the_same_urlsafe()
        {
            var tests = new[]
            {
                "AACxF-bAEAA",
            ...
            foreach (var test in tests)
            {
                var l = IdConverter.ToUrlSafeLong(test);
                var s = IdConverter.ToUrlSafeString(l);
                Assert.Equal(test, s);
                _output.WriteLine("{0} => {1}", test, l);
            }
        }
